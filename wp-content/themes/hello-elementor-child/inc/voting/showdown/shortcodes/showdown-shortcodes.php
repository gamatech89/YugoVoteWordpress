...
function yuv_showdown_shortcode($atts) {
    ...
    if (!$showdown) {
        ...
        if (!$showdown) {
            return '<div class="sd-empty-wrapper"><div class="sd-empty"><div class="sd-empty__icon"><i class="ri-sword-line"></i></div><h3 class="sd-empty__title">Nema aktivnih Showdown-ova</h3><p class="sd-empty__desc">Trenutno nema aktivnih takmičenja. Navrati ponovo uskoro!</p></div></div>';
        }
    }
    ...
    if (count($items) < 2) {
        return '<div class="sd-empty-wrapper"><div class="sd-empty"><div class="sd-empty__icon"><i class="ri-error-warning-line"></i></div><h3 class="sd-empty__title">Nedovoljno učesnika</h3><p class="sd-empty__desc">Showdown nema dovoljno učesnika za početak.</p></div></div>';
    }
    ...
    <div class="yuv-showdown-scope">
        ...
        <div class="sd-page" 
            ...
            <header class="showdown-header">
                <div class="showdown-badge"><i class="ri-sword-line"></i> Showdown Arena</div>
                ...
                <div class="sd-fighter" id="sd-fighter-a" data-side="left">
                    ...
                    <div class="sd-fighter__content">
                        <div class="sd-fighter__info-drawer">
                            ...
                            <button class="sd-fighter__pick" data-side="left"><i class="ri-trophy-line"></i> Izaberi</button>
                        </div>
                    </div>
                </div>
                ...
                <div class="sd-fighter" id="sd-fighter-b" data-side="right">
                    ...
                    <div class="sd-fighter__content">
                        <div class="sd-fighter__info-drawer">
                            ...
                            <button class="sd-fighter__pick" data-side="right"><i class="ri-trophy-line"></i> Izaberi</button>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <p class="sd-hint" id="sd-hint">
                <i class="ri-cursor-line"></i> Klikni na karticu ili dugme da izabereš pobednika
            </p>
        </div><!-- /.sd-page -->
        ...
        <div id="sd-results" class="sd-results-screen <?php echo ($has_played || $status === 'completed') ? 'sd-results-screen--visible' : ''; ?>">
            <?php if ($has_played || $status === 'completed'): ?>
                <header class="showdown-header sd-results-header">
                    <div class="showdown-badge"><i class="ri-trophy-fill"></i> Rezultati</div>
                    <h1 class="showdown-title"><?php echo esc_html($showdown->post_title); ?></h1>
                        ...
                        ?>
                            <div class="sd-podium__item sd-podium__item--<?php echo $class; ?>">
                                <div class="sd-podium__avatar-wrap">
                                    <?php if ($class === 'gold'): ?>
                                        <div class="sd-podium__crown"><svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M5 16L3 5L8.5 10L12 4L15.5 10L21 5L19 16H5Z" fill="currentColor"/></svg></div>
                                    <?php endif; ?>
...
function yuv_showdown_archive_shortcode($atts) {
    ...
    if (empty($archive)) {
        return '<div class="sd-empty-wrapper"><div class="sd-empty"><div class="sd-empty__icon"><i class="ri-archive-line"></i></div><h3 class="sd-empty__title">Nema prethodnih Showdown-ova</h3><p class="sd-empty__desc">Još uvek nema završenih takmičenja.</p></div></div>';
    }

    ob_start();
    ?>
    <div class="yuv-showdown-scope yuv-sd-dark">
        <header class="showdown-header">
            <div class="showdown-badge"><i class="ri-archive-line"></i> Arhiva</div>
            ...
            <?php foreach ($archive as $item): ?>
                <article class="sd-archive-card">
                    ...
                    <div class="sd-archive-card__footer">
                        <span class="sd-archive-card__participants"><i class="ri-group-line"></i> <?php echo $item['total_sessions']; ?> glasača</span>
                        <a href="<?php echo esc_url(get_permalink($item['id'])); ?>" class="sd-archive-card__play">Pogledaj <i class="ri-arrow-right-s-line"></i></a>
                    </div>
                </article>
            <?php endforeach; ?>
        </div>
...
function yuv_showdown_widget_shortcode($atts) {
    ...
    <div class="yuv-showdown-scope yuv-sd-dark">
        <div class="sd-widget">
            ...
            <p class="sd-widget__desc"><?php echo count($items); ?> učesnika · <?php echo $total_players; ?> glasača</p>
            <?php 
            $cta_text = 'Pogledaj Pobednika';
            $cta_icon = 'ri-trophy-line';
            if ($is_active && !$has_played) { $cta_text = 'Igraj Sada'; $cta_icon = 'ri-sword-line'; }
            elseif ($is_active && $has_played) { $cta_text = 'Pogledaj Rezultate'; $cta_icon = 'ri-bar-chart-2-line'; }
            ?>
            <a href="<?php echo get_permalink($showdown_id); ?>" class="sd-widget__cta">
                <i class="<?php echo $cta_icon; ?>"></i> <?php echo $cta_text; ?>
            </a>
        </div>
    </div>
    <?php
