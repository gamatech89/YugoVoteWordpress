...
(function () {
  ...
  function renderState(s) {
    ...
    const pct = s.tokens.max
      ? Math.min(100, Math.round((100 * s.tokens.current) / s.tokens.max))
      : 0;
  ...
  if (elSpendBtn) {
    elSpendBtn.addEventListener("click", async () => {
      ...
      try {
        ...
        if (elSpendRes)
          elSpendRes.textContent = e && e.message ? e.message : "Greška.";
      } finally {
  ...
  if (elStartBtn) {
    elStartBtn.addEventListener("click", async () => {
      ...
      try {
        ...
        if (elStartRes)
          elStartRes.textContent = `Pokušaj #${r.attempt_id} je kreiran. Srećno!`;
        if (elAttemptId) elAttemptId.value = r.attempt_id;
        await load();
      } catch (e) {
        if (elStartRes)
          elStartRes.textContent = e && e.message ? e.message : "Greška.";
      } finally {
  ...
  if (elSubmitBtn) {
    elSubmitBtn.addEventListener("click", async () => {
      ...
      if (!id || !a || isNaN(c) || !t) {
        if (elSubmitRes)
          elSubmitRes.textContent =
            "Popunite ID kviza, attempt, tačnih i ukupno.";
        return;
      }
      ...
      try {
        ...
        if (elSubmitRes)
          elSubmitRes.textContent =
            (r.passed ? "Položeno" : "Nije položeno") +
            ` · ${r.score_percent}% · +${r.xp_awarded} XP`;
        await load();
      } catch (e) {
        if (elSubmitRes)
          elSubmitRes.textContent = e && e.message ? e.message : "Greška.";
      } finally {
